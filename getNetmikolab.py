...
def get_subnet(device_params, intf):
    ...
    try:
        cdiraddress = re.search(
            r'  Internet address is ([0-9./]+)', data).groups()
...
def get_description(device_params, intf):
    ...
    for line in result[1:]:
        line += ' '
        intf_type, intf_num, _, _, description = re.search(
            r'(\w)\w([0-9/]+)\s+(admin down|\w+)\s+(\w+)([0-9a-zA-Z\s/]+|null)', line).groups()
        if intf_type == intf[0] and intf_num == intf[1:]:
...
def get_status(device_params, intf):
    ...
    for line in result[1:]:
        line += ' '
        intf_type, intf_num, status, _, _ = re.search(
            r'(\w)\w([0-9/]+)\s+(admin down|\w+)\s+(\w+)([0-9a-zA-Z\s/]+|null)', line).groups()
        if intf_type == intf[0] and intf_num == intf[1:]:
            return status
